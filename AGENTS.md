...
- Inline `>` links → `Link` (Tailwind utilities, polymorphic `<a>`). Bracketed links → `Keycap` (`[label]`, variants `primary` lime / `secondary` slate, polymorphic `<a>`/`<button>`). There is no `Button` component — new variants go in the file's own `STYLES`/`VARIANTS` const, not a CSS-module class.
...
- **Custom scrollbar is in `base.css`** — lime thumb on dark surface, hard-edged (`border-radius: 0`) with a 1px hairline, 16px wide. Both `::-webkit-scrollbar` and `scrollbar-color` / `scrollbar-width` Firefox properties are set. The reduced-motion media query does not touch it.
...
## Accessibility
- All interactive elements are real `<a>` / `<button>`. Focus rings: 2px solid lime, `outline-offset: 2px` (global in `base.css`). `AsciiFrameAnimator` `hover` and `click` triggers expose a `focus-visible` outline so keyboard users get the same affordance.
