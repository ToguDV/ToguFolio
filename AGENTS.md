...
- **Shooting stars live in `useAsciiConstellation`** — randomized spawner (first at 2.6–4.8s, then every 7–14s); a `*` head with a fading glyph trail (`= - . , '`), brightening nearby links and twinkling/pushing nearby nodes as it passes. It reports `onShootingStar(durationMs)` (stored in a ref so the main effect doesn't re-run); `Hero` uses it to swap the cat to `CAT_HERO_STAR_FRAMES` (`data/animals.js`, starry `*` eyes) while the star crosses. Stars spawn only inside `update`, so reduced motion means no shooting stars and the cat never changes expression.
